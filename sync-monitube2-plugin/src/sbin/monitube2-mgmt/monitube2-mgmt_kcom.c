/*
 * $Id: monitube2-mgmt_kcom.c 407596 2010-10-29 10:52:37Z sunilbasker $
 *
...
ifd_down_async_handler(kcom_ifdev_t * msg, void * user_info __unused)
{
    ...
    if((msg->ifdev_op == KCOM_DELETE || junos_kcom_ifd_down(msg)) &&
       strstr(name, MSP_IFD_NAME_PATTERN)) {
        ...
        junos_trace(MONITUBE_TRACEFLAG_KCOM,
...
kcom_init(evContext ctx)
{
    ...
    if(status != 0) {
        LOG(TRACE_LOG_ERR, "%s: Retrieving origin id failed", __func__);
        return -1;
    }
    
    status = junos_kcom_init_extensive(id, ctx, &params);
...
kcom_shutdown(void)
{
    junos_trace(MONITUBE_TRACEFLAG_KCOM, "%s", __func__);
