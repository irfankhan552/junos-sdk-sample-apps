...
fclose( fp );
    ...
    */
    ...
    search_for_handle( cnx, tag, inet_ntoa(cnx->info.client_addr.sin_addr), 
        nb_variables, vars_names, vars_values );

    /* END MODIFIED LIBRARY HERE */
 ...
 * 
 * @param ctx opaque context structure as returned by mihl_init()
 * @param tag HTTP base URL (such as  â€œ/â€? or â€˜/nextpage.htmlâ€?).
 ...
 * 
 * @param ctx opaque context structure as returned by mihl_init()
 * @param tag HTTP base URL (such as â€œ/image.jpgâ€? for instance)
 * @param filename ilename to send. The full pathname can be given.
 * @param content_type HTTP content type, such as â€œimage/jpegâ€?, â€œimage/gifâ€?, â€œtext/javascriptâ€?, etc.
 ...
 * 
 * MIHL is based on a non-blocking and single thread mode; therefore youâ€™ll have to call this function 
 ...
 * each of these functions should not take too much time: the next callback function will be â€˜scheduledâ€™ 
 * only when the current function is done (sort of a  â€˜cooperative multi-taskingâ€™).
 * 
 * @param ctx opaque context structure as returned by mihl_init()
