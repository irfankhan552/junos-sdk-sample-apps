/*
 * $Id: jnx-routeserviced_ui.cc 346460 2009-11-14 05:06:47Z ssiano $
 *
 ...
 *
 * Command line requests to jnx-routeservice from the host are delivered
 * and processed here.
 */
...
#include JNX_CC_ROUTESERVICED_ODL_H
...
static void
jnx_routeserviced_show_rt_data (rs_config_data_t *rt_data, 
				mgmt_sock_t *msp,
				int32_t detail_level __unused)
{
    /*
     * Fetch the ssd instance
     */
     ...
     */
    if (rt_data->req_status == rs_ssd.get_success_status()) {
        XML_ELT(msp, ODCI_JNX_ROUTESERVICE_ROUTE_INFO_STATUS, "%s", "Success");
    } else if (rt_data->req_status == rs_ssd.get_failure_status()) {
        XML_ELT(msp, ODCI_JNX_ROUTESERVICE_ROUTE_INFO_STATUS, "%s", "Failure");
    } else if (rt_data->req_status == rs_ssd.get_pending_status()) {
			     ...
			     char *unparsed __unused)
{
     ...
     */
     for (rt_data = rs_cfg.data_first(); rt_data;
         rt_data = rs_cfg.data_next( rt_data)) {
