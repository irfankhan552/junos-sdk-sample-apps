/*
 * $Id: counterd_config.c 346460 2009-11-14 05:06:47Z ssiano $
 *
 ...
 */
PATNODE_TO_STRUCT(data_entry, counter_data_t, node)
 ...
 */
static int
delete_message (counter_data_t *data)
{
 ...
 *      the message to add
 * 
 * @return pointer to the new data entry added
 */
static counter_data_t *
add_message (const char * message)
{
    counter_data_t *data;

    /* allocate & initialize data */
    data = calloc(1, sizeof(counter_data_t)); /* calloc zeros mem */
 ...
 *****************************************************************************/
counter_data_t *
next_message (counter_data_t * data)
{
 ...
 */
int
counterd_config_read (int check)
{

    ddl_handle_t * dop = NULL; /* DDL Object Pointer */
    const char   * data_config_name[] =  { DDLNAME_SYNC,
                                           DDLNAME_SYNC_COUNTER,
                                           DDLNAME_SYNC_COUNTER_MESSAGE,
                                           NULL};
    char          msg[MESSAGE_STR_SIZE];
    int           status = SUCCESS;

    /*
     * read counterd data configuration
     */
    if (dax_get_object_by_path(NULL, data_config_name, &dop, FALSE)) {
        ...
        if (dax_is_changed(dop)) {
            ...
            if (dax_get_stringr_by_aid(dop, COUNTER_MESSAGE_CONTENTS, 
                    msg, sizeof(msg))) {
    
                if(add_message(msg) == NULL) { /* add this message */
                    dax_error(dop, "Failed to add message to configuration");
                    status = EFAIL;
                } else if(!check) {
                    reset_message(msg); /* message added, GENCFG reset */
                }
